...
int main(){  
    ...
    le(ay,"janei-1.pgm");  
    Mat_<COR> qx; 
    le(qx,"julho_mv_mediana.pgm");
    if(ax.size()!=ay.size()) 
        erro("Erro dimensao");  
        
    Mat_<GRY> qp(qx.rows,qx.cols);//Cria as estruturas de dados para alimentar OpenCV  
    
    Mat_<FLT> features(ax.rows*ax.cols,3);  
    Mat_<FLT> saidas(ax.rows*ax.cols,1);
    
    int i=0;for(int l=0; l<ax.rows; l++)
    ...
    for(int l=0; l<qp.rows; l++)
        for(int c=0; c<qp.cols; c++){      
            ...
            query(2)=qx(l,c)[2]/255.0;// Zero indica sem backtracking      
